...
# initialize states for SIR:
# mutually exclusive positive and negative I and R states
...
# run single SIR simulation given parameters
def dual_sir_single(
    G,
    beta_pos,
    beta_neg,
    ...
    # initialize pos and negative node sets

    state = {u: S for u in G.nodes()}
    ever_pos = set()
    ever_neg = set()

    # initialize seeds with negative or positive sentiment
    for u in seeds_pos:
        state[u] = I_POS
        ever_pos.add(u)
    for u in seeds_neg:
    ...
    # helper function to get neighbors for directed and undirected situations
    def neigh(u):
        if directed and isinstance(G, nx.DiGraph):
            return G.predecessors(u)
        return G.neighbors(u)

    # initialize history tracking for graphs

    hist_ratio = []
    hist_inf_pos = []
    hist_inf_neg = []

    # run simulation (timestep loop)

    for _ in range(max_steps):

        # get count of nodes in state to add to history tracking
        ...
        # resolve conflicts between neg and pos : mutually exclusive
    ...
    # return history and total negative and positive nodes
...
# network/graph loading helper function
...
# helper to get top 100 nodes in a network
def influencer_seeds(G, k=100):
    deg_sorted = sorted(G.degree(), key=lambda x: x[1], reverse=True)
    return [n for n, _ in deg_sorted[:k]]

# helper to average a series over every timestep and return the average series
...
# plotting helper for total infected graph
...
# plotting helper for ratio graph
...
if __name__ == "__main__":
    ...
    # run all sets of parameter simulations for all networks, 15 times each
    # average the series of all 15 runs over all timesteps to plot
